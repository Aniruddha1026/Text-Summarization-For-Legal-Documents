...
class LegalDocumentSummarizer:
    ...
    def summarize_chunk(self, text, max_length=150, min_length=50):
        """Summarize a single chunk of text using GPU if available."""
        inputs = self.tokenizer("summarize: " + text, return_tensors="pt", max_length=1024, truncation=True).to(self.device)
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            file_path = input("\nEnter the path to the .docx file: ").strip().strip('"').strip("'")
            try:
            ...
            file_path = input("\nEnter the path to the .txt file: ").strip().strip('"').strip("'")
